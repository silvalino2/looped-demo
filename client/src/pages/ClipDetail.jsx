              ...
              <div>
                <div style={{ background: '#0d0d0d', border: '1px solid #1a1a1a', borderRadius: 16, overflow: 'hidden' }}>
                  ...
                  <div style={{ padding: '20px 24px', borderTop: '1px solid #1a1a1a' }}>
                    ...
                    {(activeClip.caption || activeClip.hashtags?.length > 0) && (
                      <div style={{ marginTop: 20, borderTop: '1px solid #1a1a1a', paddingTop: 20 }}>
                        ...
                        {activeClip.caption && (
                          <div style={{ marginBottom: 16 }}>
                            ...
                            <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.8)', lineHeight: 1.6, background: '#111', padding: '14px 16px', borderRadius: 10, border: '1px solid #1e1e1e' }}>
                              {activeClip.caption}
                            </p>
                          </div>
                        )}
                        ...
                        {activeClip.hashtags?.length > 0 && (
                          <div style={{ marginBottom: 16 }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                              <p style={{ fontSize: 11, fontWeight: 600, color: 'rgba(255,255,255,0.3)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>Hashtags</p>
                              <button
                                onClick={() => copyCaption(activeClip.hashtags.map(h => `#${h.replace(/^#/, '')}`).join(' '), `hashtags-${activeClip.id}`)}
                                style={{ fontSize: 11, background: 'none', border: 'none', cursor: 'pointer', color: copied === `hashtags-${activeClip.id}` ? '#86efac' : 'rgba(255,255,255,0.4)', fontFamily: "'DM Sans', sans-serif", fontWeight: 500, transition: 'color 200ms' }}
                              >
                                {copied === `hashtags-${activeClip.id}` ? '✓ Copied' : 'Copy'}
                              </button>
                            </div>
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                              {activeClip.hashtags.map((tag, i) => (
                                <span key={i} style={{ fontSize: 12, padding: '4px 10px', background: 'rgba(134,239,172,0.08)', border: '1px solid rgba(134,239,172,0.2)', borderRadius: 20, color: '#86efac', fontWeight: 500 }}>
                                  #{tag.replace(/^#/, '')}
                                </span>
                              ))}
                            </div>
                        ...
                        {activeClip.caption && activeClip.hashtags?.length > 0 && (
                          <button
                            onClick={() => copyCaption(
                              `${activeClip.caption}\n\n${activeClip.hashtags.map(h => `#${h.replace(/^#/, '')}`).join(' ')}`,
                              `all-${activeClip.id}`
                            )}
                            style={{ width: '100%', padding: '10px', background: 'rgba(134,239,172,0.06)', border: '1px solid rgba(134,239,172,0.15)', borderRadius: 10, color: copied === `all-${activeClip.id}` ? '#86efac' : 'rgba(255,255,255,0.5)', fontSize: 12, fontWeight: 500, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif", transition: 'all 200ms' }}
                          >
                            {copied === `all-${activeClip.id}` ? '✓ Copied everything' : 'Copy caption + hashtags'}
                          </button>
                        )}
                      </div>
                    )}
