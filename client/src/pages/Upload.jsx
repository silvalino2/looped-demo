...
const STAGES_UPLOAD = [
  ...
  { key: 'transcribing', label: 'Analysing content' },
...
const STAGE_MAP_UPLOAD = { extracting: 1, transcribing: 2, segmenting: 3, rendering: 4, completed: 5 }
const STAGE_MAP_YOUTUBE = { downloading: 0, extracting: 1, transcribing: 2, segmenting: 3, rendering: 4, completed: 5 }
...
// Parse mm:ss or plain seconds input
const parseTime = (val) => {
  if (!val) return 0
  if (val.includes(':')) {
...
function SettingsPanel({ settings, onChange }) {
  ...
  const isCustomDuration = settings.clipDurationPreset === 'custom'

  const handleDurationPreset = (val) => {
    if (val === 'custom') {
      onChange({ clipDurationPreset: 'custom' })
    } else {
      onChange({ clipDurationPreset: val, clipDuration: val })
    }
  }
  ...
  const handleCustomDuration = (val) => {
    ...
    if (n > 0) onChange({ clipDuration: n })
  }

  const handleStartChange = (val) => {
    setStartInput(val)
    onChange({ startAt: parseTime(val) })
  }

  const handleEndChange = (val) => {
    setEndInput(val)
    onChange({ endAt: parseTime(val) || undefined })
  }
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {CLIP_DURATION_PRESETS.map(p => (
            <ToggleButton
              key={p.value}
              active={settings.clipDurationPreset === p.value}
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {MAX_CLIPS_PRESETS.map(n => (
            <ToggleButton
              key={n}
              active={settings.maxClips === n}
              onClick={() => onChange({ maxClips: n })}
            >
              {n}
            </ToggleButton>
          ))}
      ...
      <div>
        <label style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)', display: 'block', marginBottom: 10 }}>
          Time range <span style={{ color: 'rgba(255,255,255,0.25)', fontWeight: 400 }}>(optional — clip only a section)</span>
        </label>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', letterSpacing: '0.05em' }}>FROM</span>
            <input
              type="text"
              placeholder="0:00"
              value={startInput}
              ...
              style={{
                width: 80, padding: '8px 12px', background: '#111',
                border: '1px solid #2a2a2a', borderRadius: 8, color: '#fff',
                fontSize: 13, fontFamily: "'DM Mono', monospace", outline: 'none', textAlign: 'center'
              }}
              onFocus={e => e.target.style.borderColor = 'rgba(134,239,172,0.4)'}
              onBlur={e => e.target.style.borderColor = '#2a2a2a'}
            />
          </div>
          <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.2)', paddingTop: 18 }}>→</span>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', letterSpacing: '0.05em' }}>TO</span>
            <input
              type="text"
              placeholder="end"
              value={endInput}
          ...
          <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.2)', paddingTop: 18 }}>
            format: m:ss or seconds
          </p>
        </div>
      </div>
      ...
      {/* Summary */}
      <div style={{
        marginTop: 18, padding: '10px 14px', background: 'rgba(134,239,172,0.04)',
        border: '1px solid rgba(134,239,172,0.1)', borderRadius: 10
      }}>
        <p style={{ fontSize: 12, color: 'rgba(134,239,172,0.7)', fontFamily: "'DM Mono', monospace" }}>
          Up to {settings.maxClips} clips · {isCustomDuration ? (settings.clipDuration || '?') : settings.clipDuration}s each
          {settings.startAt > 0 || settings.endAt
            ? ` · ${fmtTime(settings.startAt)} → ${settings.endAt ? fmtTime(settings.endAt) : 'end'}`
            : ' · full video'
          }
        </p>
      </div>
...
export default function Upload() {
  ...
  const handleProcessFile = async () => {
    ...
    formData.append('clipDuration', settings.clipDuration)
    formData.append('maxClips', settings.maxClips)
    formData.append('startAt', settings.startAt || 0)
    if (settings.endAt) formData.append('endAt', settings.endAt)
    ...
    try {
      ...
      setCurrentStage(1)
      await pollStatus(res.data.jobId, STAGE_MAP_UPLOAD)
      ...
      <div style={{ padding: 'clamp(28px, 4vw, 48px)', maxWidth: 720 }}>
        ...
        <div style={{ marginBottom: 32, animation: 'fadeUp 0.5s cubic-bezier(0.16,1,0.3,1) both' }}>
          ...
          <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)' }}>
            Upload a file or paste a YouTube link. Set your clip length before processing.
          </p>
        </div>
        ...
        {!processing && (
          <div style={{ animation: 'fadeUp 0.5s cubic-bezier(0.16,1,0.3,1) 60ms both' }}>
                ...
                {file ? (
                  <div>
                    ...
                    <button onClick={e => { e.stopPropagation(); setFile(null) }}
                      style={{ marginTop: 12, fontSize: 12, color: '#f87171', background: 'none', border: 'none', cursor: 'pointer', padding: '4px 8px', fontFamily: "'DM Sans', sans-serif" }}>
                      Remove
                    </button>
                  </div>
                ) : (
            ...
            {mode === 'youtube' && (
              <div style={{ marginBottom: 20 }}>
                <label style={{ fontSize: 12, fontWeight: 600, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.06em', textTransform: 'uppercase', display: 'block', marginBottom: 10 }}>
                  YouTube URL
                </label>
                <input
                  type="url"
                  ...
                  style={{
                    ...
                    borderRadius: 12, color: '#fff', fontSize: 14, fontFamily: "'DM Sans', sans-serif",
                ...
                <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.2)', marginTop: 8 }}>
                  Works for youtube.com/watch, youtu.be, and Shorts · requires yt-dlp on the server
                </p>
              </div>
            )}
            ...
            <Button size="lg" fullWidth
              onClick={mode === 'file' ? handleProcessFile : handleProcessYoutube}
              disabled={!canSubmit}>
            ...
            {mode === 'file' && (
              <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.2)', marginTop: 10, textAlign: 'center' }}>
                ⚠ Large files (1hr+) may time out on Railway — use the YouTube URL option instead for long videos
              </p>
            )}
          </div>
        )}
          ...
          <div style={{
            ...
            <div style={{ textAlign: 'center', marginBottom: 36 }}>
              ...
              <h2 style={{ fontSize: 18, fontWeight: 600, letterSpacing: '-0.02em', marginBottom: 6 }}>
                {mode === 'youtube' ? 'Downloading & processing' : 'Processing your video'}
              </h2>
              <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)' }}>
                {settings.clipDuration}s clips · up to {settings.maxClips} clips
                {settings.startAt > 0 || settings.endAt ? ` · ${fmtTime(settings.startAt)} → ${settings.endAt ? fmtTime(settings.endAt) : 'end'}` : ''}
              </p>
            </div>
